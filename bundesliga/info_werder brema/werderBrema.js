...
var xhrWerderBrema = new XMLHttpRequest();
var idSquadra = 72;
var codiceSquadra = "WBR";

xhrWerderBrema.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrWerderBrema.onload = function () {
    if (xhrWerderBrema.status === 200) {
        console.log("Risposta del server:", xhrWerderBrema.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrWerderBrema.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_werder-brema/' + codiceCalciatore.toLowerCase() + '.jpg")';
