...
var xhrFriburgo = new XMLHttpRequest();
var idSquadra = 73;
var codiceSquadra = "FRI";

xhrFriburgo.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrFriburgo.onload = function () {
    if (xhrFriburgo.status === 200) {
        console.log("Risposta del server:", xhrFriburgo.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrFriburgo.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_friburgo/' + codiceCalciatore.toLowerCase() + '.jpg")';
