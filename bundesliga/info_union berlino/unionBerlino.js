...
var xhrUnionBerlino = new XMLHttpRequest();
var idSquadra = 78;
var codiceSquadra = "UBL";

xhrUnionBerlino.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrUnionBerlino.onload = function () {
    if (xhrUnionBerlino.status === 200) {
        console.log("Risposta del server:", xhrUnionBerlino.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrUnionBerlino.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_union-berlino/' + codiceCalciatore.toLowerCase() + '.jpg")';
