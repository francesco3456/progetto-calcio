...
var xhrAugusta = new XMLHttpRequest();
var idSquadra = 74;
var codiceSquadra = "AUG";

xhrAugusta.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrAugusta.onload = function () {
    if (xhrAugusta.status === 200) {
        console.log("Risposta del server:", xhrAugusta.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrAugusta.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_augusta/' + codiceCalciatore.toLowerCase() + '.jpg")';
