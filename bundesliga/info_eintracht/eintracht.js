...
var xhrEintracht = new XMLHttpRequest();
var idSquadra = 69;
var codiceSquadra = "EIN";

xhrEintracht.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrEintracht.onload = function () {
    if (xhrEintracht.status === 200) {
        console.log("Risposta del server:", xhrEintracht.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrEintracht.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_eintracht/' + codiceCalciatore.toLowerCase() + '.jpg")';
