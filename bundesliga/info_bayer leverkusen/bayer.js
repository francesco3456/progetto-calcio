...
var xhrBayer = new XMLHttpRequest();
var idSquadra = 64;
var codiceSquadra = "BLV";

xhrBayer.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBayer.onload = function () {
    if (xhrBayer.status === 200) {
        console.log("Risposta del server:", xhrBayer.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBayer.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_bayer-leverkusen/' + codiceCalciatore.toLowerCase() + '.jpg")';
