...
var xhrStoccarda = new XMLHttpRequest();
var idSquadra = 65;
var codiceSquadra = "STO";

xhrStoccarda.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrStoccarda.onload = function () {
    if (xhrStoccarda.status === 200) {
        console.log("Risposta del server:", xhrStoccarda.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrStoccarda.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_stoccarda/' + codiceCalciatore.toLowerCase() + '.jpg")';
