...
var xhrHolsteinKiel = new XMLHttpRequest();
var idSquadra = 80;
var codiceSquadra = "KIE";

xhrHolsteinKiel.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrHolsteinKiel.onload = function () {
    if (xhrHolsteinKiel.status === 200) {
        console.log("Risposta del server:", xhrHolsteinKiel.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrHolsteinKiel.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_Holstein-kiel/' + codiceCalciatore.toLowerCase() + '.jpg")';
