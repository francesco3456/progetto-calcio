function toBack() {
    window.location.href = "file:///C:/Users/franc/Documents/progetto-Calcio-versione-microservizi/frontend/bundesliga/bundesliga.html";
}
...
var xhrBorussiaDortmund = new XMLHttpRequest();
var idSquadra = 68;
var codiceSquadra = "BOD";

xhrBorussiaDortmund.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBorussiaDortmund.onload = function () {
    if (xhrBorussiaDortmund.status === 200) {
        console.log("Risposta del server:", xhrBorussiaDortmund.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBorussiaDortmund.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_borussia-dortmund/' + codiceCalciatore.toLowerCase() + '.jpg")';
