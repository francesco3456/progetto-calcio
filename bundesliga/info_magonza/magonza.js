...
var xhrMagonza = new XMLHttpRequest();
var idSquadra = 76;
var codiceSquadra = "MAI";

xhrMagonza.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMagonza.onload = function () {
    if (xhrMagonza.status === 200) {
        console.log("Risposta del server:", xhrMagonza.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMagonza.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_magonza/' + codiceCalciatore.toLowerCase() + '.jpg")';
