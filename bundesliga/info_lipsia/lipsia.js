...
var xhrLipsia = new XMLHttpRequest();
var idSquadra = 67;
var codiceSquadra = "LIP";

xhrLipsia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLipsia.onload = function () {
    if (xhrLipsia.status === 200) {
        console.log("Risposta del server:", xhrLipsia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLipsia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_lipsia/' + codiceCalciatore.toLowerCase() + '.jpg")';
