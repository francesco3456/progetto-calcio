...
var xhrWolfsburg = new XMLHttpRequest();
var idSquadra = 75;
var codiceSquadra = "WLB";

xhrWolfsburg.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrWolfsburg.onload = function () {
    if (xhrWolfsburg.status === 200) {
        console.log("Risposta del server:", xhrWolfsburg.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrWolfsburg.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_wolfsburg/' + codiceCalciatore.toLowerCase() + '.jpg")';
