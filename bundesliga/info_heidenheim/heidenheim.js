...
var xhrHeidenheim = new XMLHttpRequest();
var idSquadra = 71;
var codiceSquadra = "HEI";

xhrHeidenheim.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrHeidenheim.onload = function () {
    if (xhrHeidenheim.status === 200) {
        console.log("Risposta del server:", xhrHeidenheim.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrHeidenheim.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_heidenheim/' + codiceCalciatore.toLowerCase() + '.jpg")';
