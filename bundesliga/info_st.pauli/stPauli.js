...
var xhrStPauli = new XMLHttpRequest();
var idSquadra = 79;
var codiceSquadra = "STP";

xhrStPauli.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrStPauli.onload = function () {
    if (xhrStPauli.status === 200) {
        console.log("Risposta del server:", xhrStPauli.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrStPauli.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_st.pauli/' + codiceCalciatore.toLowerCase() + '.jpg")';
