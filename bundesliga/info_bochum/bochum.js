...
var xhrBochum = new XMLHttpRequest();
var idSquadra = 81;
var codiceSquadra = "BOC";

xhrBochum.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBochum.onload = function () {
    if (xhrBochum.status === 200) {
        console.log("Risposta del server:", xhrBochum.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBochum.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_bochum/' + codiceCalciatore.toLowerCase() + '.jpg")';
