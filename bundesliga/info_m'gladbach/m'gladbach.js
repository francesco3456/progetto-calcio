...
var xhrMgladbach = new XMLHttpRequest();
var idSquadra = 77;
var codiceSquadra = "BMG";

xhrMgladbach.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMgladbach.onload = function () {
    if (xhrMgladbach.status === 200) {
        console.log("Risposta del server:", xhrMgladbach.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMgladbach.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_m\'gladbach/' + codiceCalciatore.toLowerCase() + '.jpg")';
