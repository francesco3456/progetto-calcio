...
var xhrHoffenheim = new XMLHttpRequest();
var idSquadra = 70;
var codiceSquadra = "HOF";

xhrHoffenheim.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrHoffenheim.onload = function () {
    if (xhrHoffenheim.status === 200) {
        console.log("Risposta del server:", xhrHoffenheim.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrHoffenheim.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_hoffenheim/' + codiceCalciatore.toLowerCase() + '.jpg")';
