...
var xhrBayern = new XMLHttpRequest();
var idSquadra = 66;
var codiceSquadra = "BAY";

xhrBayern.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBayern.onload = function () {
    if (xhrBayern.status === 200) {
        console.log("Risposta del server:", xhrBayern.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBayern.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_bayern-monaco/' + codiceCalciatore.toLowerCase() + '.jpg")';
