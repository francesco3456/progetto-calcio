...
var xhrRealMadrid = new XMLHttpRequest();
var idSquadra = 100;
var codiceSquadra = "RMA";

xhrRealMadrid.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrRealMadrid.onload = function () {
    if (xhrRealMadrid.status === 200) {
        console.log("Risposta del server:", xhrRealMadrid.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrRealMadrid.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_real-madrid/' + codiceCalciatore.toLowerCase() + '.jpg")';
