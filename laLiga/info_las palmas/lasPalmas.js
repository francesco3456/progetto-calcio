...
var xhrLasPalmas = new XMLHttpRequest();
var idSquadra = 115;
var codiceSquadra = "LAP";

xhrLasPalmas.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLasPalmas.onload = function () {
    if (xhrLasPalmas.status === 200) {
        console.log("Risposta del server:", xhrLasPalmas.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLasPalmas.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_las-palmas/' + codiceCalciatore.toLowerCase() + '.jpg")';
