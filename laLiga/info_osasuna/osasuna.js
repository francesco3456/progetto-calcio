...
var xhrOsasuna = new XMLHttpRequest();
var idSquadra = 110;
var codiceSquadra = "OSA";

xhrOsasuna.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrOsasuna.onload = function () {
    if (xhrOsasuna.status === 200) {
        console.log("Risposta del server:", xhrOsasuna.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrOsasuna.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_osasuna/' + codiceCalciatore.toLowerCase() + '.jpg")';
