...
var xhrLeganes = new XMLHttpRequest();
var idSquadra = 118;
var codiceSquadra = "LGN";

xhrLeganes.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLeganes.onload = function () {
    if (xhrLeganes.status === 200) {
        console.log("Risposta del server:", xhrLeganes.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLeganes.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_leganes/' + codiceCalciatore.toLowerCase() + '.jpg")';
