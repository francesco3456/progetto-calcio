...
var xhrCeltaVigo = new XMLHttpRequest();
var idSquadra = 112;
var codiceSquadra = "CEV";

xhrCeltaVigo.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCeltaVigo.onload = function () {
    if (xhrCeltaVigo.status === 200) {
        console.log("Risposta del server:", xhrCeltaVigo.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCeltaVigo.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_celta-vigo/' + codiceCalciatore.toLowerCase() + '.jpg")';
