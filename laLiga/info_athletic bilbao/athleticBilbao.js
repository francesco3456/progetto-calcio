function toBack() {
    window.location.href = "file:///C:/Users/franc/Documents/progetto-Calcio-versione-microservizi/frontend/laLiga/laLiga.html";
}
...
var xhrAthleticBilbao = new XMLHttpRequest();
var idSquadra = 104;
var codiceSquadra = "ATB";

xhrAthleticBilbao.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrAthleticBilbao.onload = function () {
    if (xhrAthleticBilbao.status === 200) {
        console.log("Risposta del server:", xhrAthleticBilbao.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrAthleticBilbao.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_athletic-bilbao/' + codiceCalciatore.toLowerCase() + '.jpg")';
