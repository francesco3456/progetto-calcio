...
var xhrMaiorca = new XMLHttpRequest();
var idSquadra = 114;
var codiceSquadra = "MRC";

xhrMaiorca.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMaiorca.onload = function () {
    if (xhrMaiorca.status === 200) {
        console.log("Risposta del server:", xhrMaiorca.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMaiorca.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_Maiorca/' + codiceCalciatore.toLowerCase() + '.jpg")';
