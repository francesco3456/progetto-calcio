...
var xhrRealValladolid = new XMLHttpRequest();
var idSquadra = 117;
var codiceSquadra = "RVA";

xhrRealValladolid.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrRealValladolid.onload = function () {
    if (xhrRealValladolid.status === 200) {
        console.log("Risposta del server:", xhrRealValladolid.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrRealValladolid.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_real-valladolid/' + codiceCalciatore.toLowerCase() + '.jpg")';
