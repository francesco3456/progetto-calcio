...
var xhrGirona = new XMLHttpRequest();
var idSquadra = 102;
var codiceSquadra = "GIR";

xhrGirona.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrGirona.onload = function () {
    if (xhrGirona.status === 200) {
        console.log("Risposta del server:", xhrGirona.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrGirona.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_girona/' + codiceCalciatore.toLowerCase() + '.jpg")';
