...
var xhrEspanyol = new XMLHttpRequest();
var idSquadra = 119;
var codiceSquadra = "ESP";

xhrEspanyol.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrEspanyol.onload = function () {
    if (xhrEspanyol.status === 200) {
        console.log("Risposta del server:", xhrEspanyol.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrEspanyol.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_espanyol/' + codiceCalciatore.toLowerCase() + '.jpg")';
