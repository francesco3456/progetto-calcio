...
var xhrRealSociedad = new XMLHttpRequest();
var idSquadra = 105;
var codiceSquadra = "RSO";

xhrRealSociedad.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrRealSociedad.onload = function () {
    if (xhrRealSociedad.status === 200) {
        console.log("Risposta del server:", xhrRealSociedad.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrRealSociedad.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_real-sociedad/' + codiceCalciatore.toLowerCase() + '.jpg")';
