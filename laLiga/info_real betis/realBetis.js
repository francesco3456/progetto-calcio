...
var xhrRealBetis = new XMLHttpRequest();
var idSquadra = 106;
var codiceSquadra = "RBE";

xhrRealBetis.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrRealBetis.onload = function () {
    if (xhrRealBetis.status === 200) {
        console.log("Risposta del server:", xhrRealBetis.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrRealBetis.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_real-betis/' + codiceCalciatore.toLowerCase() + '.jpg")';
