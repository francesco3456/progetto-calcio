...
var xhrBarcelona = new XMLHttpRequest();
var idSquadra = 101;
var codiceSquadra = "BRC";

xhrBarcelona.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBarcelona.onload = function () {
    if (xhrBarcelona.status === 200) {
        console.log("Risposta del server:", xhrBarcelona.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBarcelona.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_barcelona/' + codiceCalciatore.toLowerCase() + '.jpg")';
