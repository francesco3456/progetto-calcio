...
var xhrSiviglia = new XMLHttpRequest();
var idSquadra = 113;
var codiceSquadra = "SIV";

xhrSiviglia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrSiviglia.onload = function () {
    if (xhrSiviglia.status === 200) {
        console.log("Risposta del server:", xhrSiviglia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrSiviglia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_siviglia/' + codiceCalciatore.toLowerCase() + '.jpg")';
