...
var xhrRayoVallecano = new XMLHttpRequest();
var idSquadra = 116;
var codiceSquadra = "RAY";

xhrRayoVallecano.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrRayoVallecano.onload = function () {
    if (xhrRayoVallecano.status === 200) {
        console.log("Risposta del server:", xhrRayoVallecano.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrRayoVallecano.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_rayo-vallecano/' + codiceCalciatore.toLowerCase() + '.jpg")';
