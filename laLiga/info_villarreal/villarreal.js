...
var xhrVillarreal = new XMLHttpRequest();
var idSquadra = 107;
var codiceSquadra = "VIL";

xhrVillarreal.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrVillarreal.onload = function () {
    if (xhrVillarreal.status === 200) {
        console.log("Risposta del server:", xhrVillarreal.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrVillarreal.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_villarreal/' + codiceCalciatore.toLowerCase() + '.jpg")';
