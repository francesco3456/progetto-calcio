...
var xhrGetafe = new XMLHttpRequest();
var idSquadra = 111;
var codiceSquadra = "GET";

xhrGetafe.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrGetafe.onload = function () {
    if (xhrGetafe.status === 200) {
        console.log("Risposta del server:", xhrGetafe.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrGetafe.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_getafe/' + codiceCalciatore.toLowerCase() + '.jpg")';
