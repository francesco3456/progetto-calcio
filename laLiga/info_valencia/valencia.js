...
var xhrValencia = new XMLHttpRequest();
var idSquadra = 108;
var codiceSquadra = "VAL";

xhrValencia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrValencia.onload = function () {
    if (xhrValencia.status === 200) {
        console.log("Risposta del server:", xhrValencia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrValencia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_valencia/' + codiceCalciatore.toLowerCase() + '.jpg")';
