...
var xhrAlaves = new XMLHttpRequest();
var idSquadra = 109;
var codiceSquadra = "ALA";

xhrAlaves.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrAlaves.onload = function () {
    if (xhrAlaves.status === 200) {
        console.log("Risposta del server:", xhrAlaves.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrAlaves.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_d.alaves/' + codiceCalciatore.toLowerCase() + '.jpg")';
