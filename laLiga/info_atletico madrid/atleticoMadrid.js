...
var xhratleticoMadrid = new XMLHttpRequest();
var idSquadra = 103;
var codiceSquadra = "ATM";

xhratleticoMadrid.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhratleticoMadrid.onload = function () {
    if (xhratleticoMadrid.status === 200) {
        console.log("Risposta del server:", xhratleticoMadrid.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhratleticoMadrid.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_athletic-madrid/' + codiceCalciatore.toLowerCase() + '.jpg")';
