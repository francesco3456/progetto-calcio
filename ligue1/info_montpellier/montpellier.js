function toBack() {
    window.location.href = "file:///C:/Users/franc/Documents/progetto-Calcio-versione-microservizi/frontend/ligue1/ligue1.html";
}
...
var xhrMontpellier = new XMLHttpRequest();
var idSquadra = 93;
var codiceSquadra = "MTP";

xhrMontpellier.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMontpellier.onload = function () {
    if (xhrMontpellier.status === 200) {
        console.log("Risposta del server:", xhrMontpellier.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMontpellier.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_montpellier/' + codiceCalciatore.toLowerCase() + '.jpg")';
