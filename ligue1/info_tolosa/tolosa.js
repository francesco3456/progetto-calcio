...
var xhrTolosa = new XMLHttpRequest();
var idSquadra = 92;
var codiceSquadra = "TOL";

xhrTolosa.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrTolosa.onload = function () {
    if (xhrTolosa.status === 200) {
        console.log("Risposta del server:", xhrTolosa.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrTolosa.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_tolosa/' + codiceCalciatore.toLowerCase() + '.jpg")';
