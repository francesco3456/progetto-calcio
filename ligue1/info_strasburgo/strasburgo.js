...
var xhrStrasburgo = new XMLHttpRequest();
var idSquadra = 94;
var codiceSquadra = "STR";

xhrStrasburgo.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrStrasburgo.onload = function () {
    if (xhrStrasburgo.status === 200) {
        console.log("Risposta del server:", xhrStrasburgo.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrStrasburgo.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_strasburgo/' + codiceCalciatore.toLowerCase() + '.jpg")';
