...
var xhrParisSG = new XMLHttpRequest();
var idSquadra = 82;
var codiceSquadra = "PSG";

xhrParisSG.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrParisSG.onload = function () {
    if (xhrParisSG.status === 200) {
        console.log("Risposta del server:", xhrParisSG.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrParisSG.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_paris-sg/' + codiceCalciatore.toLowerCase() + '.jpg")';
