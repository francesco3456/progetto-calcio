...
var xhrAuxerre = new XMLHttpRequest();
var idSquadra = 98;
var codiceSquadra = "AUX";

xhrAuxerre.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrAuxerre.onload = function () {
    if (xhrAuxerre.status === 200) {
        console.log("Risposta del server:", xhrAuxerre.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrAuxerre.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_auxerre/' + codiceCalciatore.toLowerCase() + '.jpg")';
