...
var xhrMarsiglia = new XMLHttpRequest();
var idSquadra = 89;
var codiceSquadra = "MAR";

xhrMarsiglia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMarsiglia.onload = function () {
    if (xhrMarsiglia.status === 200) {
        console.log("Risposta del server:", xhrMarsiglia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMarsiglia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_marsiglia/' + codiceCalciatore.toLowerCase() + '.jpg")';
