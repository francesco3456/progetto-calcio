...
var xhrLens = new XMLHttpRequest();
var idSquadra = 88;
var codiceSquadra = "LEN";

xhrLens.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLens.onload = function () {
    if (xhrLens.status === 200) {
        console.log("Risposta del server:", xhrLens.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLens.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_lens/' + codiceCalciatore.toLowerCase() + '.jpg")';
