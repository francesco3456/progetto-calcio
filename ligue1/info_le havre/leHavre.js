...
var xhrLeHavre = new XMLHttpRequest();
var idSquadra = 96;
var codiceSquadra = "LEH";

xhrLeHavre.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLeHavre.onload = function () {
    if (xhrLeHavre.status === 200) {
        console.log("Risposta del server:", xhrLeHavre.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLeHavre.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_le-havre/' + codiceCalciatore.toLowerCase() + '.jpg")';
