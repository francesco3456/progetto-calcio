...
var xhrNantes = new XMLHttpRequest();
var idSquadra = 95;
var codiceSquadra = "NAN";

xhrNantes.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrNantes.onload = function () {
    if (xhrNantes.status === 200) {
        console.log("Risposta del server:", xhrNantes.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrNantes.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_nantes/' + codiceCalciatore.toLowerCase() + '.jpg")';
