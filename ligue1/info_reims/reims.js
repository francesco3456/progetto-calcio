...
var xhrReims = new XMLHttpRequest();
var idSquadra = 90;
var codiceSquadra = "REI";

xhrReims.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrReims.onload = function () {
    if (xhrReims.status === 200) {
        console.log("Risposta del server:", xhrReims.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrReims.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_reims/' + codiceCalciatore.toLowerCase() + '.jpg")';
