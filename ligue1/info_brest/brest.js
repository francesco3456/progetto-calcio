...
var xhrBrest = new XMLHttpRequest();
var idSquadra = 84;
var codiceSquadra = "BST";

xhrBrest.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBrest.onload = function () {
    if (xhrBrest.status === 200) {
        console.log("Risposta del server:", xhrBrest.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBrest.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_brest/' + codiceCalciatore.toLowerCase() + '.jpg")';
