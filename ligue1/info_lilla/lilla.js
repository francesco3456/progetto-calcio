...
var xhrLilla = new XMLHttpRequest();
var idSquadra = 85;
var codiceSquadra = "LIL";

xhrLilla.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLilla.onload = function () {
    if (xhrLilla.status === 200) {
        console.log("Risposta del server:", xhrLilla.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLilla.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_lilla/' + codiceCalciatore.toLowerCase() + '.jpg")';
