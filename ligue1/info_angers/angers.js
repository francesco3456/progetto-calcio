...
var xhrAngers = new XMLHttpRequest();
var idSquadra = 99;
var codiceSquadra = "ANG";

xhrAngers.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrAngers.onload = function () {
    if (xhrAngers.status === 200) {
        console.log("Risposta del server:", xhrAngers.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrAngers.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_angers/' + codiceCalciatore.toLowerCase() + '.jpg")';
