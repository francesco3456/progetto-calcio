...
var xhrStEtienne = new XMLHttpRequest();
var idSquadra = 97;
var codiceSquadra = "STE";

xhrStEtienne.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrStEtienne.onload = function () {
    if (xhrStEtienne.status === 200) {
        console.log("Risposta del server:", xhrStEtienne.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrStEtienne.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_st.etienne/' + codiceCalciatore.toLowerCase() + '.jpg")';
