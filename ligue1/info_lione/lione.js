...
var xhrLione = new XMLHttpRequest();
var idSquadra = 87;
var codiceSquadra = "LIO";

xhrLione.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLione.onload = function () {
    if (xhrLione.status === 200) {
        console.log("Risposta del server:", xhrLione.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLione.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_lione/' + codiceCalciatore.toLowerCase() + '.jpg")';
