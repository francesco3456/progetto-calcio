...
var xhrMonaco = new XMLHttpRequest();
var idSquadra = 83;
var codiceSquadra = "MNC";

xhrMonaco.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMonaco.onload = function () {
    if (xhrMonaco.status === 200) {
        console.log("Risposta del server:", xhrMonaco.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMonaco.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_monaco/' + codiceCalciatore.toLowerCase() + '.jpg")';
