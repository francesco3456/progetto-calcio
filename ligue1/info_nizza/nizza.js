...
var xhrNizza = new XMLHttpRequest();
var idSquadra = 86;
var codiceSquadra = "NIZ";

xhrNizza.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrNizza.onload = function () {
    if (xhrNizza.status === 200) {
        console.log("Risposta del server:", xhrNizza.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrNizza.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_nizza/' + codiceCalciatore.toLowerCase() + '.jpg")';
