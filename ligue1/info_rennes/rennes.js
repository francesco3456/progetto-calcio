...
var xhrRennes = new XMLHttpRequest();
var idSquadra = 91;
var codiceSquadra = "REN";

xhrRennes.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrRennes.onload = function () {
    if (xhrRennes.status === 200) {
        console.log("Risposta del server:", xhrRennes.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrRennes.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_rennes/' + codiceCalciatore.toLowerCase() + '.jpg")';
