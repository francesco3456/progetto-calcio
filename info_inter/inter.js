...
var xhrInter = new XMLHttpRequest();
var idSquadra = 1;
var codiceSquadra = "INT";

xhrInter.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrInter.onload = function () {
    if (xhrInter.status === 200) {
        console.log("Risposta del server:", xhrInter.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrInter.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_inter/' + codiceCalciatore.toLowerCase() + '.jpg")';
