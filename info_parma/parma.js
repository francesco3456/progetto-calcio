...
var xhrParma = new XMLHttpRequest();
var idSquadra = 23;
var codiceSquadra = "PAR";

xhrParma.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrParma.onload = function () {
    if (xhrParma.status === 200) {
        console.log("Risposta del server:", xhrParma.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrParma.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_parma/' + codiceCalciatore.toLowerCase() + '.jpg")';
