...
var xhrVenezia = new XMLHttpRequest();
var idSquadra = 24;
var codiceSquadra = "VEN";

xhrVenezia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrVenezia.onload = function () {
    if (xhrVenezia.status === 200) {
        console.log("Risposta del server:", xhrVenezia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrVenezia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_venezia/' + codiceCalciatore.toLowerCase() + '.jpg")';
