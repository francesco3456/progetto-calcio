...
var xhrLazio = new XMLHttpRequest();
var idSquadra = 10;
var codiceSquadra = "LAZ";

xhrLazio.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLazio.onload = function () {
    if (xhrLazio.status === 200) {
        console.log("Risposta del server:", xhrLazio.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLazio.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_lazio/' + codiceCalciatore.toLowerCase() + '.jpg")';
