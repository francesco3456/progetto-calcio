...
var xhrLecce = new XMLHttpRequest();
var idSquadra = 18;
var codiceSquadra = "LEC";

xhrLecce.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLecce.onload = function () {
    if (xhrLecce.status === 200) {
        console.log("Risposta del server:", xhrLecce.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLecce.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_lecce/' + codiceCalciatore.toLowerCase() + '.jpg")';
