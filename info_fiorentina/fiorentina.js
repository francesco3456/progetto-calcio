...
var xhrFiorentina = new XMLHttpRequest();
var idSquadra = 12;
var codiceSquadra = "FIO";

xhrFiorentina.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrFiorentina.onload = function () {
    if (xhrFiorentina.status === 200) {
        console.log("Risposta del server:", xhrFiorentina.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrFiorentina.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_fiorentina/' + codiceCalciatore.toLowerCase() + '.jpg")';
