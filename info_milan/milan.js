...
var xhrMilan = new XMLHttpRequest();
var idSquadra = 5;
var codiceSquadra = "MIL";

xhrMilan.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMilan.onload = function () {
    if (xhrMilan.status === 200) {
        console.log("Risposta del server:", xhrMilan.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMilan.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_milan/' + codiceCalciatore.toLowerCase() + '.jpg")';
