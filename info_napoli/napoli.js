...
var xhrNapoli = new XMLHttpRequest();
var idSquadra = 14;
var codiceSquadra = "NAP";

xhrNapoli.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrNapoli.onload = function () {
    if (xhrNapoli.status === 200) {
        console.log("Risposta del server:", xhrNapoli.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrNapoli.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_napoli/' + codiceCalciatore.toLowerCase() + '.jpg")';
