...
var xhrMonza = new XMLHttpRequest();
var idSquadra = 16;
var codiceSquadra = "MON";

xhrMonza.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMonza.onload = function () {
    if (xhrMonza.status === 200) {
        console.log("Risposta del server:", xhrMonza.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMonza.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_monza/' + codiceCalciatore.toLowerCase() + '.jpg")';
