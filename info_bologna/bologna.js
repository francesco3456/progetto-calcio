...
var xhrBologna = new XMLHttpRequest();
var idSquadra = 11;
var codiceSquadra = "BOL";

xhrBologna.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBologna.onload = function () {
    if (xhrBologna.status === 200) {
        console.log("Risposta del server:", xhrBologna.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBologna.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_bologna/' + codiceCalciatore.toLowerCase() + '.jpg")';
