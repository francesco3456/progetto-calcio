...
var xhrGenoa = new XMLHttpRequest();
var idSquadra = 15;
var codiceSquadra = "GEN";

xhrGenoa.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrGenoa.onload = function () {
    if (xhrGenoa.status === 200) {
        console.log("Risposta del server:", xhrGenoa.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrGenoa.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_genoa/' + codiceCalciatore.toLowerCase() + '.jpg")';
