...
var xhrTottenham = new XMLHttpRequest();
var idSquadra = 48;
var codiceSquadra = "TOT";

xhrTottenham.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrTottenham.onload = function () {
    if (xhrTottenham.status === 200) {
        console.log("Risposta del server:", xhrTottenham.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrTottenham.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_tottenham/' + codiceCalciatore.toLowerCase() + '.jpg")';
