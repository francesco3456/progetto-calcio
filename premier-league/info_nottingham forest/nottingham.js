...
var xhrNottingham = new XMLHttpRequest();
var idSquadra = 60;
var codiceSquadra = "NOT";

xhrNottingham.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrNottingham.onload = function () {
    if (xhrNottingham.status === 200) {
        console.log("Risposta del server:", xhrNottingham.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrNottingham.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_nottingham/' + codiceCalciatore.toLowerCase() + '.jpg")';
