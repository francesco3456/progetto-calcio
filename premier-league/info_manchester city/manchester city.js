function toBack() {
    window.location.href = "file:///C:/Users/franc/Documents/progetto-Calcio-versione-microservizi/frontend/premier-league/premierLeague.html";
}
...
var xhrManchesterCity = new XMLHttpRequest();
var idSquadra = 44;
var codiceSquadra = "MCC";

xhrManchesterCity.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrManchesterCity.onload = function () {
    if (xhrManchesterCity.status === 200) {
        console.log("Risposta del server:", xhrManchesterCity.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrManchesterCity.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_manchester-city/' + codiceCalciatore.toLowerCase() + '.jpg")';
