...
var xhrIpswich = new XMLHttpRequest();
var idSquadra = 62;
var codiceSquadra = "IPS";

xhrIpswich.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrIpswich.onload = function () {
    if (xhrIpswich.status === 200) {
        console.log("Risposta del server:", xhrIpswich.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrIpswich.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_ipswich/' + codiceCalciatore.toLowerCase() + '.jpg")';
