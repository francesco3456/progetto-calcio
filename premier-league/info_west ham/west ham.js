...
var xhrWestHam = new XMLHttpRequest();
var idSquadra = 52;
var codiceSquadra = "WHU";

xhrWestHam.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrWestHam.onload = function () {
    if (xhrWestHam.status === 200) {
        console.log("Risposta del server:", xhrWestHam.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrWestHam.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_west-ham/' + codiceCalciatore.toLowerCase() + '.jpg")';
