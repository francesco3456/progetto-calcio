...
var xhrChelsea = new XMLHttpRequest();
var idSquadra = 49;
var codiceSquadra = "CHE";

xhrChelsea.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrChelsea.onload = function () {
    if (xhrChelsea.status === 200) {
        console.log("Risposta del server:", xhrChelsea.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrChelsea.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_chelsea/' + codiceCalciatore.toLowerCase() + '.jpg")';
