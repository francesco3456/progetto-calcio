...
var xhrSouthampton = new XMLHttpRequest();
var idSquadra = 63;
var codiceSquadra = "SOU";

xhrSouthampton.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrSouthampton.onload = function () {
    if (xhrSouthampton.status === 200) {
        console.log("Risposta del server:", xhrSouthampton.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrSouthampton.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_southampton/' + codiceCalciatore.toLowerCase() + '.jpg")';
