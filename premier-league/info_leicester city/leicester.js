...
var xhrLeicester = new XMLHttpRequest();
var idSquadra = 61;
var codiceSquadra = "LEI";

xhrLeicester.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLeicester.onload = function () {
    if (xhrLeicester.status === 200) {
        console.log("Risposta del server:", xhrLeicester.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLeicester.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_leicester/' + codiceCalciatore.toLowerCase() + '.jpg")';
