...
var xhrAstonVilla = new XMLHttpRequest();
var idSquadra = 47;
var codiceSquadra = "ATV";

xhrAstonVilla.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrAstonVilla.onload = function () {
    if (xhrAstonVilla.status === 200) {
        console.log("Risposta del server:", xhrAstonVilla.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrAstonVilla.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_aston-villa/' + codiceCalciatore.toLowerCase() + '.jpg")';
