...
var xhrBrentford = new XMLHttpRequest();
var idSquadra = 59;
var codiceSquadra = "BTF";

xhrBrentford.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBrentford.onload = function () {
    if (xhrBrentford.status === 200) {
        console.log("Risposta del server:", xhrBrentford.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrArsenal.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_brentford/' + codiceCalciatore.toLowerCase() + '.jpg")';
