...
var xhrEverton = new XMLHttpRequest();
var idSquadra = 58;
var codiceSquadra = "EVE";

xhrEverton.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrEverton.onload = function () {
    if (xhrEverton.status === 200) {
        console.log("Risposta del server:", xhrEverton.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrEverton.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_everton/' + codiceCalciatore.toLowerCase() + '.jpg")';
