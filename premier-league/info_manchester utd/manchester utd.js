...
var xhrManchesterUtd = new XMLHttpRequest();
var idSquadra = 51;
var codiceSquadra = "UTD";

xhrManchesterUtd.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrManchesterUtd.onload = function () {
    if (xhrManchesterUtd.status === 200) {
        console.log("Risposta del server:", xhrManchesterUtd.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrManchesterUtd.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_manchester-utd/' + codiceCalciatore.toLowerCase() + '.jpg")';
