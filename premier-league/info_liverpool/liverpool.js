...
var xhrLiverpool = new XMLHttpRequest();
var idSquadra = 46;
var codiceSquadra = "LIV";

xhrLiverpool.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrLiverpool.onload = function () {
    if (xhrLiverpool.status === 200) {
        console.log("Risposta del server:", xhrLiverpool.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrLiverpool.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_liverpool/' + codiceCalciatore.toLowerCase() + '.jpg")';
