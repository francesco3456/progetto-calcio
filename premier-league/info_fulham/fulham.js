...
var xhrFulham = new XMLHttpRequest();
var idSquadra = 56;
var codiceSquadra = "FUL";

xhrFulham.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrFulham.onload = function () {
    if (xhrFulham.status === 200) {
        console.log("Risposta del server:", xhrFulham.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrFulham.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_fulham/' + codiceCalciatore.toLowerCase() + '.jpg")';
