...
var xhrBrighton = new XMLHttpRequest();
var idSquadra = 54;
var codiceSquadra = "BRI";

xhrBrighton.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBrighton.onload = function () {
    if (xhrBrighton.status === 200) {
        console.log("Risposta del server:", xhrBrighton.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBrighton.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_brighton/' + codiceCalciatore.toLowerCase() + '.jpg")';
