...
var xhrNewcastle = new XMLHttpRequest();
var idSquadra = 50;
var codiceSquadra = "NEW";

xhrNewcastle.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrNewcastle.onload = function () {
    if (xhrNewcastle.status === 200) {
        console.log("Risposta del server:", xhrNewcastle.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrNewcastle.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_newcastle/' + codiceCalciatore.toLowerCase() + '.jpg")';
