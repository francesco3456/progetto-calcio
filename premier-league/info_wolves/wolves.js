...
var xhrWolves = new XMLHttpRequest();
var idSquadra = 57;
var codiceSquadra = "WOL";

xhrWolves.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrWolves.onload = function () {
    if (xhrWolves.status === 200) {
        console.log("Risposta del server:", xhrWolves.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrWolves.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_wolves/' + codiceCalciatore.toLowerCase() + '.jpg")';
