...
var xhrBournemouth = new XMLHttpRequest();
var idSquadra = 55;
var codiceSquadra = "BOU";

xhrBournemouth.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBournemouth.onload = function () {
    if (xhrBournemouth.status === 200) {
        console.log("Risposta del server:", xhrBournemouth.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBournemouth.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_bournemouth/' + codiceCalciatore.toLowerCase() + '.jpg")';
