...
var xhrCrystalPalace = new XMLHttpRequest();
var idSquadra = 53;
var codiceSquadra = "CRP";

xhrCrystalPalace.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCrystalPalace.onload = function () {
    if (xhrCrystalPalace.status === 200) {
        console.log("Risposta del server:", xhrCrystalPalace.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCrystalPalace.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_crystal-palace/' + codiceCalciatore.toLowerCase() + '.jpg")';
