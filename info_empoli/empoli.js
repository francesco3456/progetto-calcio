...
var xhrEmpoli = new XMLHttpRequest();
var idSquadra = 21;
var codiceSquadra = "EMP";

xhrEmpoli.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrEmpoli.onload = function () {
    if (xhrEmpoli.status === 200) {
        console.log("Risposta del server:", xhrEmpoli.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrEmpoli.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_empoli/' + codiceCalciatore.toLowerCase() + '.jpg")';
