...
var xhrSassuolo = new XMLHttpRequest();
var idSquadra = 42;
var codiceSquadra = "SAS";

xhrSassuolo.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrSassuolo.onload = function () {
    if (xhrSassuolo.status === 200) {
        console.log("Risposta del server:", xhrSassuolo.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrSassuolo.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_sassuolo/' + codiceCalciatore.toLowerCase() + '.jpg")';
