...
var xhrBrescia = new XMLHttpRequest();
var idSquadra = 29;
var codiceSquadra = "BRE";

xhrBrescia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBrescia.onload = function () {
    if (xhrBrescia.status === 200) {
        console.log("Risposta del server:", xhrBrescia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBrescia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_brescia/' + codiceCalciatore.toLowerCase() + '.jpg")';
