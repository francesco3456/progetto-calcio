...
var xhrCarrarese = new XMLHttpRequest();
var idSquadra = 120;
var codiceSquadra = "CAR";

xhrCarrarese.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCarrarese.onload = function () {
    if (xhrCarrarese.status === 200) {
        console.log("Risposta del server:", xhrCarrarese.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCarrarese.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_carrarese/' + codiceCalciatore.toLowerCase() + '.jpg")';
