...
var xhrCosenza = new XMLHttpRequest();
var idSquadra = 37;
var codiceSquadra = "COS";

xhrCosenza.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCosenza.onload = function () {
    if (xhrCosenza.status === 200) {
        console.log("Risposta del server:", xhrCosenza.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCosenza.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_cosenza/' + codiceCalciatore.toLowerCase() + '.jpg")';
