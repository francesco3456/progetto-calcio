...
var xhrMantova = new XMLHttpRequest();
var idSquadra = 40;
var codiceSquadra = "MAN";

xhrMantova.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrMantova.onload = function () {
    if (xhrMantova.status === 200) {
        console.log("Risposta del server:", xhrMantova.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrMantova.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_mantova/' + codiceCalciatore.toLowerCase() + '.jpg")';
