...
var xhrSpezia = new XMLHttpRequest();
var idSquadra = 35;
var codiceSquadra = "SPE";

xhrSpezia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrSpezia.onload = function () {
    if (xhrSpezia.status === 200) {
        console.log("Risposta del server:", xhrSpezia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrSpezia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_spezia/' + codiceCalciatore.toLowerCase() + '.jpg")';
