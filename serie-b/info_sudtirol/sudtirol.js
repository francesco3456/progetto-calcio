...
var xhrSudtirol = new XMLHttpRequest();
var idSquadra = 33;
var codiceSquadra = "SDT";

xhrSudtirol.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrSudtirol.onload = function () {
    if (xhrSudtirol.status === 200) {
        console.log("Risposta del server:", xhrSudtirol.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrSudtirol.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_sudtirol/' + codiceCalciatore.toLowerCase() + '.jpg")';
