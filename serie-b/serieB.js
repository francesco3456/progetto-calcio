...
xhr.open('GET', 'http://localhost:8765/lega-b-italiana/giornata', true);
...
xhr.onload = function () {
    if (xhr.status === 200) {
        ...
        var menuList = document.getElementById('menu-list-b');
...
xhrClassifica.open('GET', 'http://localhost:8765/lega-b-italiana/punteggi', true);
...
xhrPartite.open('GET', 'http://localhost:8765/lega-b-italiana/ultime-partite', true);
...
function aggiornaTabellaClassifica(idTabella, data) {
    ...
    data.forEach(match => {
        ...
        let linkSquadra = document.createElement("a");
        linkSquadra.textContent = nomeSquadra;
        linkSquadra.href = `info_${match.squadra.nomeSquadra}/${match.squadra.nomeSquadra}.html`;
        cellSquadra.appendChild(linkSquadra);
        row.appendChild(cellSquadra);

        let cellPunti = document.createElement("td");
        cellPunti.textContent = match.classifica.puntiSquadra || '0';
        row.appendChild(cellPunti);

        let cellVittorie = document.createElement("td");
        cellVittorie.textContent = match.classifica.vittorieSquadra || '0';
        row.appendChild(cellVittorie);

        let cellPareggi = document.createElement("td");
        cellPareggi.textContent = match.classifica.pareggiSquadra || '0';
        row.appendChild(cellPareggi);

        let cellSconfitte = document.createElement("td");
        cellSconfitte.textContent = match.classifica.sconfitteSquadra || '0';
        row.appendChild(cellSconfitte);

        let cellGolFatti = document.createElement("td");
        cellGolFatti.textContent = match.classifica.golFattiSquadra || '0';
        row.appendChild(cellGolFatti);

        let cellGolSubiti = document.createElement("td");
        cellGolSubiti.textContent = match.classifica.golSubitiSquadra || '0';
        row.appendChild(cellGolSubiti);

        let cellDifferenzaReti = document.createElement("td");
        cellDifferenzaReti.textContent = match.classifica.differenzaRetiSquadra || '0';
...
function aggiornaPartitePerGiornata(idCampionato, giornata) {
    ...
    xhrGiornata.open('GET', 'http://localhost:8765/lega-b-italiana/partite-giornata/' + encodeURIComponent(giornata) + '/' + encodeURIComponent(idCampionato), true);
