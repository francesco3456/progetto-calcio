...
var xhrBari = new XMLHttpRequest();
var idSquadra = 36;
var codiceSquadra = "BAR";

xhrBari.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrBari.onload = function () {
    if (xhrBari.status === 200) {
        console.log("Risposta del server:", xhrBari.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrBari.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_bari/' + codiceCalciatore.toLowerCase() + '.jpg")';
