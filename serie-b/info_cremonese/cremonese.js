...
var xhrCremonese = new XMLHttpRequest();
var idSquadra = 25;
var codiceSquadra = "CRE";

xhrCremonese.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCremonese.onload = function () {
    if (xhrCremonese.status === 200) {
        console.log("Risposta del server:", xhrCremonese.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCremonese.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_cremonese/' + codiceCalciatore.toLowerCase() + '.jpg")';
