function toBack() {
    window.location.href = "file:///C:/Users/franc/Documents/progetto-Calcio-versione-microservizi/frontend/serie-b/serieB.html";
}
...
var xhrSalernitana = new XMLHttpRequest();
var idSquadra = 41;
var codiceSquadra = "SAL";

xhrSalernitana.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrSalernitana.onload = function () {
    if (xhrSalernitana.status === 200) {
        console.log("Risposta del server:", xhrSalernitana.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrSalernitana.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_salernitana/' + codiceCalciatore.toLowerCase() + '.jpg")';
