...
var xhrPalermo = new XMLHttpRequest();
var idSquadra = 27;
var codiceSquadra = "PAL";

xhrPalermo.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrPalermo.onload = function () {
    if (xhrPalermo.status === 200) {
        console.log("Risposta del server:", xhrPalermo.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrPalermo.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_palermo/' + codiceCalciatore.toLowerCase() + '.jpg")';
