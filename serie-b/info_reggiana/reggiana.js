...
var xhrReggiana = new XMLHttpRequest();
var idSquadra = 31;
var codiceSquadra = "REG";

xhrReggiana.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrReggiana.onload = function () {
    if (xhrReggiana.status === 200) {
        console.log("Risposta del server:", xhrReggiana.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrReggiana.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_reggiana/' + codiceCalciatore.toLowerCase() + '.jpg")';
