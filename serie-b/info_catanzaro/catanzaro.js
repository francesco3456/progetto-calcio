...
var xhrCatanzaro = new XMLHttpRequest();
var idSquadra = 26;
var codiceSquadra = "CTZ";

xhrCatanzaro.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCatanzaro.onload = function () {
    if (xhrCatanzaro.status === 200) {
        console.log("Risposta del server:", xhrCatanzaro.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCatanzaro.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_catanzaro/' + codiceCalciatore.toLowerCase() + '.jpg")';
