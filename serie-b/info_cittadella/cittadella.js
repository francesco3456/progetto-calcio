...
var xhrCittadella = new XMLHttpRequest();
var idSquadra = 34;
var codiceSquadra = "CIT";

xhrCittadella.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCittadella.onload = function () {
    if (xhrCittadella.status === 200) {
        console.log("Risposta del server:", xhrCittadella.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCittadella.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_cittadella/' + codiceCalciatore.toLowerCase() + '.jpg")';
