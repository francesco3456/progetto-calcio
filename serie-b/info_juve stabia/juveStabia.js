...
var xhrJuveStabia = new XMLHttpRequest();
var idSquadra = 38;
var codiceSquadra = "JVS";

xhrJuveStabia.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrJuveStabia.onload = function () {
    if (xhrJuveStabia.status === 200) {
        console.log("Risposta del server:", xhrJuveStabia.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrJuveStabia.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_juve-stabia/' + codiceCalciatore.toLowerCase() + '.jpg")';
