...
var xhrCesena = new XMLHttpRequest();
var idSquadra = 39;
var codiceSquadra = "CES";

xhrCesena.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCesena.onload = function () {
    if (xhrCesena.status === 200) {
        console.log("Risposta del server:", xhrCesena.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCesena.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_cesena/' + codiceCalciatore.toLowerCase() + '.jpg")';
