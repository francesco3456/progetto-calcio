...
var xhrModena = new XMLHttpRequest();
var idSquadra = 30;
var codiceSquadra = "MOD";

xhrModena.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrModena.onload = function () {
    if (xhrModena.status === 200) {
        console.log("Risposta del server:", xhrModena.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrModena.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_modena/' + codiceCalciatore.toLowerCase() + '.jpg")';
