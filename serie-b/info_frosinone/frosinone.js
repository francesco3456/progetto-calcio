...
var xhrFrosinone = new XMLHttpRequest();
var idSquadra = 43;
var codiceSquadra = "FRO";

xhrFrosinone.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrFrosinone.onload = function () {
    if (xhrFrosinone.status === 200) {
        console.log("Risposta del server:", xhrFrosinone.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrFrosinone.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_frosinone/' + codiceCalciatore.toLowerCase() + '.jpg")';
