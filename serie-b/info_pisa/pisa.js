...
var xhrPisa = new XMLHttpRequest();
var idSquadra = 32;
var codiceSquadra = "PIS";

xhrPisa.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrPisa.onload = function () {
    if (xhrPisa.status === 200) {
        console.log("Risposta del server:", xhrPisa.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrPisa.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_pisa/' + codiceCalciatore.toLowerCase() + '.jpg")';
