...
var xhrSampdoria = new XMLHttpRequest();
var idSquadra = 28;
var codiceSquadra = "SAM";

xhrSampdoria.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrSampdoria.onload = function () {
    if (xhrSampdoria.status === 200) {
        console.log("Risposta del server:", xhrSampdoria.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrSampdoria.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_sampdoria/' + codiceCalciatore.toLowerCase() + '.jpg")';
