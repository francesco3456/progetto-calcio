...
var xhrTorino = new XMLHttpRequest();
var idSquadra = 13;
var codiceSquadra = "TOR";

xhrTorino.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrTorino.onload = function () {
    if (xhrTorino.status === 200) {
        console.log("Risposta del server:", xhrTorino.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrTorino.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_torino/' + codiceCalciatore.toLowerCase() + '.jpg")';
