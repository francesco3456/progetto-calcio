...
var xhrUdinese = new XMLHttpRequest();
var idSquadra = 19;
var codiceSquadra = "UDI";

xhrUdinese.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrUdinese.onload = function () {
    if (xhrUdinese.status === 200) {
        console.log("Risposta del server:", xhrUdinese.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrUdinese.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_udinese/' + codiceCalciatore.toLowerCase() + '.jpg")';
