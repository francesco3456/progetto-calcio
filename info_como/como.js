...
var xhrComo = new XMLHttpRequest();
var idSquadra = 22;
var codiceSquadra = "COM";

xhrComo.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrComo.onload = function () {
    if (xhrComo.status === 200) {
        console.log("Risposta del server:", xhrComo.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrComo.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_como/' + codiceCalciatore.toLowerCase() + '.jpg")';
