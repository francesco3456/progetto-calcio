...
var xhrCagliari = new XMLHttpRequest();
var idSquadra = 20;
var codiceSquadra = "CAG";

xhrCagliari.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrCagliari.onload = function () {
    if (xhrCagliari.status === 200) {
        console.log("Risposta del server:", xhrCagliari.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrCagliari.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_cagliari/' + codiceCalciatore.toLowerCase() + '.jpg")';
