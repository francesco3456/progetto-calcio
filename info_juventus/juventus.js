...
var xhrJuventus = new XMLHttpRequest();
var idSquadra = 7;
var codiceSquadra = "JUV";

xhrJuventus.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrJuventus.onload = function () {
    if (xhrJuventus.status === 200) {
        console.log("Risposta del server:", xhrJuventus.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrJuventus.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_juventus/' + codiceCalciatore.toLowerCase() + '.jpg")';
