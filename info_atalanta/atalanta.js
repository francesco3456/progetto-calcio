...
var xhrAtalanta = new XMLHttpRequest();
var idSquadra = 8;
var codiceSquadra = "ATA";

xhrAtalanta.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrAtalanta.onload = function () {
    if (xhrAtalanta.status === 200) {
        console.log("Risposta del server:", xhrAtalanta.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrAtalanta.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_atalanta/' + codiceCalciatore.toLowerCase() + '.jpg")';
