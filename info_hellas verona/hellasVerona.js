...
var xhrHellasVerona = new XMLHttpRequest();
var idSquadra = 17;
var codiceSquadra = "VER";

xhrHellasVerona.open('GET', 'http://localhost:8765/recupero-stats/calciatori/' + encodeURIComponent(idSquadra) + '/' + encodeURIComponent(codiceSquadra), true);
xhrHellasVerona.onload = function () {
    if (xhrHellasVerona.status === 200) {
        console.log("Risposta del server:", xhrHellasVerona.responseText);

        // Converti la risposta JSON in un array di oggetti calciatori
        var data = JSON.parse(xhrHellasVerona.responseText);
        ...
        data.forEach(function (item, index) {
            ...
            if (codiceCalciatore) {
                ...
                calciatoreDiv.style.backgroundImage = 'url("images_hellas-verona/' + codiceCalciatore.toLowerCase() + '.jpg")';
