...
@RestController
@RequestMapping("/lega-spagnola")
public class LaLigaController {
	
	private RestTemplate restTemplate;
	
	@Autowired
    public LaLigaController(RestTemplate restTemplate) {
	...
	public List<Punteggi> getPunteggi() {
	    ...
	    RicercaResponseClassifica ricercaResponse = restTemplate.getForObject("http://localhost:9092/classifica/scelta-campionato/Spagna", RicercaResponseClassifica.class);
	...
	public List<PartitaEntity> getUltimePartite() {
		RicercaResponsePartita ricercaResponse = restTemplate.getForObject("http://localhost:9091/partite/ultime-partite/9", RicercaResponsePartita.class);
